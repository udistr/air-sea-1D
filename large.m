...
% [m s-2]     [K-1]  [J s-1 m-2][kg-1 m^3][J-1 kg K]=[m s-2] [s-1 m] = [m^2
% s-3]

L=ustar.^3./(karman.*Bf); %[m^3 s-3]*[s^3 m-2]=[m]
Cv=1.5; bt=0.2; Cs=98.96; eps=0.1;
ws_star0=(abs(Bf./Z)).^(1./3); %-sign(Bf).*
Vt2=max(Cv.*sqrt(bt)./(Rcrit.*karman.^2).*sqrt(Cs.*eps).*abs(Z)...
    .*sqrt(abs([diff(B,1) 0]./dz)).*ws_star0,0.0001); %*ws

A=(0.1.*abs(Z'))-abs(Z)>0; %matrix with the indexes of surface layer
A(:,1)=max(A(:,1),1);
N=sum(A,2);

R=(B-(sum(B.*A,2)./N)').*Z./((sqrt(U(1).^2+V(1).^2)-sqrt(U.^2+V.^2)).^2+Vt2.^2);
i=find(R>Rcrit);
if isempty(i)
    h=abs(ZM(end));
    i=length(R);
...
if Bf>0
    he=0.7.*ustar./f;
    a=(abs(Z)-he);
    ie=find(a.*(a<0)==max(a.*(a<0)));
    a=(abs(Z)-L);
    im=find(a.*(a<0)==max(a.*(a<0)));
    i=min(ie(1),i);
    i=min(im(1),i);
end
...
i1=find(sigma>=0.1 & sigma<1); i2=find(sigma<0.1);

wm=zeros(size(Z));
if ~isempty(i1); wm(i1)=karman.*ustar./kpp_psim(0.1.*xi); end
if ~isempty(i2); wm(i2)=karman.*ustar./kpp_psim(sigma(i2).*xi); end

ws=zeros(size(Z));
if ~isempty(i1); ws(i1)=karman.*ustar./kpp_psis(0.1.*xi); end
if ~isempty(i2); ws(i2)=karman.*ustar./kpp_psis(sigma(i2).*xi); end


ds=0.01;
sigmad=sigma+ds;
i1=find(sigmad>=0.1 & sigmad<1); i2=find(sigmad<0.1);

wmd=zeros(size(Z));
if ~isempty(i1); wmd(i1)=karman.*ustar./kpp_psim(0.1.*xi); end
if ~isempty(i2); wmd(i2)=karman.*ustar./kpp_psim(sigmad(i2).*xi); end

wsd=zeros(size(Z));
if ~isempty(i1); wsd(i1)=karman.*ustar./kpp_psis(0.1.*xi); end
if ~isempty(i2); wsd(i2)=karman.*ustar./kpp_psis(sigmad(i2).*xi); end

ni0=50e-4;
Ri0=0.7;
p1=3;

Rig=([-diff(B,1) 0]).*dz./[(diff(U,1).^2+diff(V,1).^2) 1];
